...
$akun = mysqli_query($koneksi, "SELECT level.level, user.* FROM user LEFT JOIN level ON level.id = user.id_level ORDER BY id DESC");
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    $delete = mysqli_query($koneksi, "DELETE FROM user WHERE id='$id'");
    header("location: akun.php?hapus=berhasil");
}
            ...
            <div class="layout-page">
                ...
                <div class="content-wrapper">
                    ...
                    <div class="container-xxl flex-grow-1 container-p-y">
                        <div class="row">
                            <div class="card">
                                <h5 class="card-header">Setting Akun</h5>
                                <div class="table-responsive text-nowrap">
                                    <div align="right">
                                        <a href="add_akun.php" class="btn btn-success"><i class="fa-solid fa-square-plus"></i>Tambah Akun</a>
                                    </div>
                                    <table class="table table-hover">
                                        ...
                                        <tbody class="table-border-bottom-0">
                                            <?php $no = 1;
                                            while ($rowAcc = mysqli_fetch_assoc($akun)) : ?>
                                                <tr>
                                                    <td><?php echo $no++ ?></td>
                                                    <td><?php echo $rowAcc['nama'] ?></td>
                                                    <td><span class="badge bg-label-primary me-1"><?php echo $rowAcc['level'] ?></td></span>
                                                    <td>| <a href="add_akun.php?edit=<?php echo $rowAcc['id'] ?>"><i class='bx bx-edit-alt'></i></a> | | <a href="akun.php?delete=<?php echo $rowAcc['id'] ?>" onclick="return confirm('Apakah anda yakin akan menghapus data ini??')"><i class='bx bx-trash'></i> |</a></td>
                                                    </td>
                                                </tr>
                                            <?php endwhile ?>
                                        </tbody>
